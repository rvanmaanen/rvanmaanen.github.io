...
description: My experiences setting up an Azure Function with Python and Visual Studio Code
published: true
---

Googling led me to a couple of blog posts about Python on Azure Functions. Those led me to believe Python on Azure Functions is using Windows as OS and uses a very old Python version.
...
Finally I went to the [Azure Functions Python Worker GitHub page](https://github.com/Azure/azure-functions-python-worker) and read something a little more inspiring: Python 3.6, Linux and the 2.x runtime :-)
So I decided to give it a try on my Windows 10 machine with Visual Studio Code.
...
Also, what you would find if you would read this to the end: I ended up with an terrible exception when running from Azure. Works on my machine, but this definately isn't production ready yet.
...
1. Create a 3.6 virtual environment inside your soon to-be project directory, by issueing this command to the Python launcher py: `py -3.6 -m venv C:\Projects\MyFunctionApp\env`. 
...
1. Start up Visual Studio code and open the folder "C:\Projects\MyFunctionApp" and click 'Yes' when asked if you want to initialize the project for optimal use with VS Code. Pick 'Python' in the next step.
...
- FIY: If you have multiple Python versions installed, when you issue the command `python --version` from your virtual environment, it will still say 3.7.0 or whatever is on your system path. The virtual environment does have it's own python executable in "Scripts\python.exe", so if you issue the command `env\Scripts\python --version` it will say 3.6.6. Not sure if or when this will be an issue. Tools such as pip (see the section about fixing missing modules) are smart enough to notice they're being run inside a virtual environment and automatically adjust.
...
1. All dependencies are listed in the requirements.txt file. You can install them by issueing this command in your Python terminal: `pip install -r .\requirements.txt`.
- If you're serious about your Python development you should have a look at [pipenv](https://docs.pipenv.org/), which addresses some issues with pip en virtual environments.
...
1. Publishing will again take quite some time (even 10+ minutes if you are still building the azure-functions-python-worker code). It is a known issue that publishing takes a long time anyway and apparently is being worked on.

# Trying it out!
1. After fetching the URL for my HTTP trigger from the Azure Portal, I opened up my favorite browser and hit the endpoint. HTTP ERROR 500.
